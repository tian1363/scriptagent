...
export async function updateSkill(id, input) {
  return request(`/api/skills/${id}`, { method: "PUT", headers: jsonHeaders, body: JSON.stringify(input) });
}
...
export async function loginOwner(username, password) { return request("/api/owner/login", { method:"POST", headers:jsonHeaders, body:JSON.stringify({ username, password }) }); }
export async function logoutOwner() { return request("/api/owner/logout", { method:"POST", headers:jsonHeaders }); }
