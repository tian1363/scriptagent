...
- 裂变脚本数量输入。
- 补充要求输入框。
- 裂变方向前置选项：按裂变数量生成 N 行单选下拉，每行对应 1 条裂变脚本，且每行只能选择 1 个裂变元素。
- 生成按钮。
- 任务进度。
- 结果 Tabs：
  - 视频分析
...
创建任务区按产品资产、参考视频、生成设置、补充要求和裂变方向分组。结果区按内容类型展示：运行日志渲染为步骤时间线，视频分析 Markdown 渲染标题和表格，JSON 结果展示脚本摘要卡片并保留原始 JSON。
