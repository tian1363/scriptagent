...
var builtInSkillCatalog = []BuiltInSkillInfo{
	{Name: "generate-video", Title: "生成视频", Description: "将当前脚本或描述生成视频，确认素材与规格后提交，并在对话中查看结果。", Category: "视频生成", InvocationPrompt: "/生成视频", Content: generateVideoSkillContent},
	{
		...
		InvocationPrompt: "调用 ugc_hook_writer skill，基于当前产品，帮我设计 3 个不同角度的 UGC 视频开头，写出首帧、动作、口播和字幕。",
		Content: strings.TrimSpace(`
# Skill: ugc_hook_writer
用途：从真实使用场景出发，为产品短视频设计具体、可拍摄的 UGC 开头，抓住目标用户的注意力并自然引出产品价值。

先确认：目标用户、平台/市场及语言、产品、目标行为、可用素材和拍摄限制。若已选择产品，先用产品资料或对话中明确提供的事实；关键卖点缺失时先问最多 2 个必要问题，也可标明假设继续出创意草案。

创作规则：
1. 从用户实际会遇到的触发时刻出发：谁、在哪里、正做什么、遇到了什么小问题；开头必须与后续产品演示有直接关系。
2. 每个方案写清 0–3 秒首帧画面/动作、自然口语的第一句、屏幕字幕，以及 3–8 秒如何接入产品演示。声音、画面和字幕要互补，不要重复堆字。
3. 至少给 3 种不同机制，例如具体困扰、意外发现、结果先行、真实疑问；只使用与目标用户和产品事实相符的机制。避免千篇一律的“你还不知道吗”。
4. 尽量用真实人物视角、手部动作、产品在用和可观察的细节。不要虚构真实用户身份、亲测经历、评价、销量、优惠、稀缺性或效果数据。
5. 钩子承诺必须在后续内容中兑现；没有证据时不写绝对化结果、医疗/安全功效、竞品贬损或无法证实的“第一/最好”。素材不能拍出时标注所需镜头或替代拍法。

输出：
- 一句话创意方向：目标人群、场景和要传达的核心价值。
- 3 个开头方案：角度｜首帧与动作｜口播第一句｜屏幕字幕｜3–8 秒承接｜所需素材/证据。
- 推荐 1 个最容易拍、最贴合卖点的方案，说明原因。
- 最小 A/B 测试：固定产品演示和 CTA，只改变一个开头元素；列出要观察的前段留存或点击指标，不预测效果。
`),
	},
	{
		Name: "product_selling_point_writer", Title: "场景化卖点表达", Category: "产品资产",
		Description:      "把可核实的产品特点转成用户场景、可见收益和简洁准确的销售文案。",
		InvocationPrompt: "调用 product_selling_point_writer skill，基于当前产品，帮我梳理卖点，设计真实使用场景，并写出准确、简洁、能拍出来的表达。",
		Content: strings.TrimSpace(`
# Skill: product_selling_point_writer
用途：把产品事实转化成让目标用户能理解、能想象、能验证的场景化卖点表达，供商品页、短视频和口播使用。

事实优先：先读取已选择产品的 Markdown、现有素材和用户明确提供的信息；区分“已证实的事实”“合理的使用场景”“待核实的推断”。没有证据的参数、功效、对比、用户评价和结果一律不补造。若关键事实缺失，列出最多 3 个具体待核实问题，并用占位说明，不把假设写成成品卖点。

工作步骤：
1. 卖点拆解：产品特点是什么；它能帮助哪类人完成什么任务；用户可观察到的变化是什么；依据来自哪段产品资料/素材；适用条件和限制是什么。
2. 场景设计：选真实高频或高痛点的时刻，明确人物、地点/时间、触发问题、使用动作、可见结果；产品必须在场景中发挥可展示的作用，而非只作为背景摆拍。
3. 文案表达：先说用户关心的结果或困扰，再用产品动作/特点解释原因。使用短句和具体名词，删掉空泛形容词；按目标市场语言自然本地化，避免逐字硬译。
4. 对每条宣称做准确性检查：是否有依据、条件是否说清、画面能否证明；把“更快/更好/最省”等比较词改成有依据的具体说法，或标为待核实。

输出：
- 核心判断：最值得先讲的 1 个卖点及对应人群。
- 卖点卡片（优先 2–3 条）：事实/资料依据｜目标用户和需求｜场景五要素｜可见收益｜限制/待核实点。
- 每条卖点的可用表达：商品页一句话、UGC 口播一句话、画面动作与字幕一句话；文案不得超出已知事实。
- 推荐主方案：按“场景触发 → 产品使用 → 可见变化”给出 3 镜头以内的短视频表达，并说明需要补拍或核实的内容。
`),
	},
	{
